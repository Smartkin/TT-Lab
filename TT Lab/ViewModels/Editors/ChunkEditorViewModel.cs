    ...
    IDirtyMarker
{
    ...
    public override async Task<Boolean> CanCloseAsync(CancellationToken cancellationToken = new())
    {
        if (IsDirty)
        {
    ...
    public SceneInstance NewSceneInstance(Type type, IAsset basedOn)
    {
    ...
    public void ChangeInstanceEditor(IAsset? asset)
    {
        if (asset == null)
        {
        ...
        if (asset.Type == typeof(Folder) || asset.Type == typeof(TT_Lab.Assets.Instance.Scenery) || asset.Type == typeof(TT_Lab.Assets.Instance.DynamicScenery))
        {
            return;
        }

        if (CurrentInstanceEditor != null && CurrentInstanceEditor.EditableResource == asset.URI)
        {
            return;
        }
        ...
        try
        {
            ...
            NotifyOfPropertyChange(nameof(CurrentInstanceEditor));
            if (_trackedAssets.ContainsKey(asset.URI))
            {
                return;
            }
                
            _dirtyTracker.AddChild(CurrentInstanceEditor);
            _trackedAssets.Add(asset.URI, CurrentInstanceEditor);
        }
        catch (Exception ex)
        {
    ...
    public void InstanceEditorChanged(ResourceTreeElementViewModel? vm)
    {
        if (vm == null)
        {
            if (CurrentInstanceEditor == null)
    ...
    public void InstanceEditorChanged(AvaloniaPropertyChangedEventArgs e)
    {
        ...
        ChangeInstanceEditor(asset.Asset);
    }

    public void SelectDifferentInstance(IAsset asset)
    {
        if (!_isChunkReady)
        {
            return;
    ...
    public void SelectDifferentInstanceFromView(SelectionChangedEventArgs e)
    {
        if (!_isChunkReady)
        {
            return;
    ...
    {
        _trackedAssets.Remove(deletedViewModel.Asset.URI);
    }
